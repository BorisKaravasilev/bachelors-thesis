...
{
	/// <summary>
	/// Multiplies textures with corresponding indexes of two lists of same length and with textures of the same size. 
	/// </summary>
	public class MultiplyTextureLists : DividableTask
	{
		...
		public MultiplyTextureLists(Func<List<Color[]>> getMultiplicandTextures, Func<List<Color[]>> getMultiplierTextures)
		{
		...
		protected override void ExecuteStep()
		{
			...
			{
				resultTexture[pixelIndex] = multiplicandTextures[textureIndex][pixelIndex] *
				                            multiplierTextures[textureIndex][pixelIndex];
			}
		...
		protected override void GetInputFromPreviousTask()
		{
			...
			{
				Debug.LogError(
					"The multiplicand or multiplier texture lists don't contain textures with the same pixel count.");
				Enabled = false;
			}
		}
